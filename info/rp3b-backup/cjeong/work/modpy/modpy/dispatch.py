...
class Dispatcher:
    ...
    def __init__(self, name, rpcip, rpcport, mcastip, mcastport, loop):
        self.name = name
        self.rpcip = rpcip
        self.rpcport = rpcport
        self.mcastip = mcastip
        self.mcastport = mcastport
        self.loop = loop
        self.input_queue = asyncio.Queue()
        self.output_queue = asyncio.Queue()
        self.codec = message.MessageCodecJSON()
        self.addr = self.rpcip + ":" + str(self.rpcport)

        self.rpc_server = CallServer(self.codec, self.rpcport,
                                     self.mcastport, self.rpcip,
                                     self.input_queue, self.output_queue,
                                     self.loop)
        
        self.rpc_client = CallClient(self.codec, self.rpcport,
                                     self.mcastip, self.mcastport,
                                     self.rpcip,
                                     self.input_queue, self.output_queue,
                                     self.loop)
    ...
    def execute_nb_coro(self, m):
        ...
        try:
            ...
            if (res == None):
                logger.log(logging.INFO, "No such resource: %s", m.body.resource)
                print("No such resource: %s", m.body.resource)
            else:
    ...
    def call_coro(self, m):
        try:
            ...
            # For BCAST messages, we don't need to handle callseq
